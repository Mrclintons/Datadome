...
# Main Function, where Cookie is genned
def cookiegen(domain):
    ...
    payload = {
        ...
        # conn = http.client.HTTPSConnection("api-js.datadome.co")

    response = requests.post("https://api-js.datadome.co/js/", headers=headers, data=payload)
    data = json.loads(response.text)
    if data['status'] == 200:
        return(data['status'],data['cookie'])
    else:
        return(data['status'],data['body'])

print(cookiegen("https://footlocker.de/"))
